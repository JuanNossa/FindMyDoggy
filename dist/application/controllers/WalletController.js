...
const Transaction_1 = require("../../domain/models/Transaction");
...
class WalletController {
    ...
    static async getWallet(req, res) {
        try {
            ...
            const wallet = await Wallet_1.Wallet.findByUserId(Number(user_id));
            if (!wallet) {
     ...
     */ static async buyCoins(req, res) {
        try {
            ...
            if (!wallet) {
                ...
                wallet = await Wallet_1.Wallet.create(new Wallet_1.Wallet(Number(user_id), 0));
            }
            ...
            await Transaction_1.Transaction.create(new Transaction_1.Transaction(wallet.id, 'credit', coins, 'Compra de coins'));
    ...
    static async transferCoins(req, res) {
        try {
            ...
            const fromWallet = await Wallet_1.Wallet.findByUserId(Number(from_user_id));
            const toWallet = await Wallet_1.Wallet.findByUserId(Number(to_user_id));
            if (!fromWallet) {
            ...
            try {
                ...
                await connection.query('INSERT INTO transactions (wallet_id, type, amount, description, created_at) VALUES (?, ?, ?, ?, NOW())', [fromWallet.id, 'debit', transferAmount, `Transferencia a usuario ${to_user_id}`]);
                // Registrar la transacción para el receptor
                await connection.query('INSERT INTO transactions (wallet_id, type, amount, description, created_at) VALUES (?, ?, ?, ?, NOW())', [toWallet.id, 'credit', transferAmount, `Transferencia desde usuario ${from_user_id}`]);
